...
const App = () => {
 ...
 // ...existing code...
  ...
  //   if (!node || !node.data || !node.data.details) {
  //     return (
  //       <Box sx={{ p: 2, textAlign: 'center', color: 'text.secondary', width: '80%' }}>
  //         <Typography variant="h6" component="p" sx={{ mb: 1, color: 'primary.main', textAlign: 'center' }}>
  //           Select a node for more information
  //         </Typography>
  //         <Typography variant="body2" sx={{ color: 'primary.dark', textAlign: 'center' }}>
  //           Click on any node in the diagram to view its details here.
  //         </Typography>
  //       </Box>
  //     );
  ...
  //   return (
  //     <Box sx={{ p: 2, position: 'relative', width: '90%', textAlign: 'center' }}>
  //       <Typography variant="h6" component="h3" sx={{ fontWeight: 'bold', mb: 1, color: 'primary.dark', textAlign: 'center' }}>
  //         Node Name: {node.data.label.split('\n')[0]}
  //       </Typography>

  //       {type === 'inputNode' && (
  //         <>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}>
  //             <strong>Type:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.type}</span>
  //           </Typography>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}>
  //             <strong>Identifier:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.identifier}</span>
  //           </Typography>
  //           {details.table && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Table:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.table}</span></Typography>}
  //           {details.schema && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Schema:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.schema}</span></Typography>}
  //           {details.format && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Format:</strong> <span style={{ fontWeight: 500 }}>{details.format}</span></Typography>}
  //           {details.path && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Path:</strong> <span style={{ wordBreak: 'break-all', fontWeight: 500 }}>{details.path}</span></Typography>}
  //           {details.option && (
  //             <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
  //               <Box
  //                 component="pre"
  //                 sx={{
  //                   bgcolor: 'linear-gradient(90deg, #e0f2fe 0%, #f0fdfa 100%)',
  ...
  //         <>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}>
  //             <strong>Input DFs:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.t_inputs}</span>
  //           </Typography>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}>
  //             <strong>Output DF:</strong> <span style={{ fontWeight: 500, color: '#06b6d4' }}>{details.output}</span>
  //           </Typography>
  //           {details.query && (
  //             <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
  //               <Box
  //                 component="pre"
  //                 sx={{
  //                   bgcolor: 'linear-gradient(90deg, #ede9fe 0%, #f0fdfa 100%)',
  //                   background: 'linear-gradient(90deg, #ede9fe 0%, #f0fdfa 100%)',
  //                   p: 2,
  //                   borderRadius: 2,
  //                   fontSize: '0.95rem',
  //                   whiteSpace: 'pre-wrap',
  ...
  //       {type === 'outputNode' && (
  //         <>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Type:</strong> <span style={{ fontWeight: 500 }}>{details.type}</span></Typography>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Identifier:</strong> <span style={{ fontWeight: 500 }}>{details.identifier}</span></Typography>
  //           <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Output Format:</strong> <span style={{ fontWeight: 500 }}>{details.output_format}</span></Typography>
  //           {details.partition && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Partition:</strong> <span style={{ fontWeight: 500 }}>{details.partition}</span></Typography>}
  //           {details.schema && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Schema:</strong> <span style={{ fontWeight: 500 }}>{details.schema}</span></Typography>}
  //           {details.table && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Table:</strong> <span style={{ fontWeight: 500 }}>{details.table}</span></Typography>}
  //           {details.path && <Typography variant="body2" sx={{ mb: 0.5, color: 'text.primary', textAlign: 'center' }}><strong>Path:</strong> <span style={{ wordBreak: 'break-all', fontWeight: 500 }}>{details.path}</span></Typography>}
  //           {details.option && (
  //             <Box sx={{ mt: 2, display: 'flex', justifyContent: 'center' }}>
  //               <Box
  //                 component="pre"
  //                 sx={{
  //                   bgcolor: 'linear-gradient(90deg, #fff7ed 0%, #f0fdfa 100%)',
  ...
  //                   boxShadow: 'inset 0 1px 3px rgba(251,146,60,0.08)',
  //                   fontFamily: 'monospace',
  //                   color: '#fb923c',
  //                   maxWidth: 400,
  //                   textAlign: 'left'
